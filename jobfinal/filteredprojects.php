...
if($level==0)
{
    ...
    $query="select * from projects p,company c where p.cid=c.cid and p.level='0' and p.is_visible='1' and p.pid NOT IN(Select pid from undergoing_project where uid='".$uid."') ";
...
{
    $query="select * from projects p,company c where p.cid=c.cid and p.level='".$level."' and p.is_visible='1' and p.pid NOT IN(Select pid from undergoing_project where uid='".$uid."' UNION select pid from completed_project where uid='".$uid."' ) order by p.name";
    $res=mysql_query($query);
            ...
            while($data=mysql_fetch_array($res))
            {
                ...
                <div class="productWrap" id="project_<?php echo $count;?>">
                    <div class="productImageWrap" id="productImageWrapID_<?php echo $count;?>">
                        ...
                        <img src="<?php echo $data[17];?>" width="100px" id="img<?php echo $data[0]; ?>" style="cursor: pointer" onclick="details(<?php echo $data[0]; ?>)" height="100px" alt="Product<?php echo $count;?>">
