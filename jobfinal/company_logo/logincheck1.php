<?php
session_start();
/*die(header("Location: index.php?value=Trials have been ended! Thank you for joining us. Main event starts on 2nd Nov. at 12.00 pm.</br> <a href= /"http://trovetrace.infotsav.org/" >Trove Trace</a> trials have started."));*/
...
/*else
{
header("Location: index.php?value=Trials have been ended! Thank you for joining us. Main event starts on 2nd Nov. at 12.00 pm.</br> <a href= /"http://trovetrace.infotsav.org/" >Trove Trace</a> trials have started.");
}
exit();
session_start();*/
/*else
{
$username = "akashdeep1412";
$pw = "ericsson1412";
if($_POST['uname']=="pri_bajaj" && $_POST['pass']=="chalokhelen:D"){
...
//mysql_select_db("infotsav12") or die("Error connecting to databases");
    ...
    //1while($rowsw=mysql_fetch_array($res3))
    //{
...
session_destroy();
die(header("Location: index.php?value=Trials have been ended! Thank you for joining us. Main event starts on 2nd Nov. at 12.00 pm."));
    ...
    else
    {
        while($rows1=mysql_fetch_array($res1))
        {
            if(md5($_POST['pass'])==$rows1[6])
            {
                ...
                $query="INSERT INTO jobbureau.profile VALUES('".$username."','".$name."','".$password."','user','".$username."','".$email."','".$college."',100,0,0,NOW(),NOW(),0,'user_pics/default.png')";
...
//header("Location: index.php?value=Register Yourself at Infotsav'12 dwd");
}
}*/
?>
